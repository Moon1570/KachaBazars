...
<body>
	...
	<div class="header">
		...
		<%			
			...
			System.out.println(request.getAttribute("upaId").toString() + "dsfkjsnjf");
			
			if(request.getAttribute("upaId").toString() != null)
			{
				int upaId = Integer.parseInt(request.getAttribute("upaId").toString());
				System.out.println("dsnfj" + upaId);
				List<UnionModel> unionModels = ad.getUnionByUpazillaId(upaId);
				
				request.setAttribute("union", unionModels);
			}
	...
	<div class="container-box big">
		<table align="center">
			...
			<tr>
	 		...
	 		<c:forEach items= "${union}" var="area">
			<tr>
				<td>${area.unionId}</td>
			<td>${area.unionBanglaName}</td>
				<td>
					<a class="blue-button small" href="./areas?action=update&aid=${area.unionId}">Edit</a>
					<a class="red-button small" href="./areas?action=delete&aid=${area.unionId}">Delete</a>
				</td>
			</tr>
			</c:forEach> 
		</table>
