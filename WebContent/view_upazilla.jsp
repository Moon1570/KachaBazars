...
<body>
	...
	<div class="header">
		...
		<%			
			...
			if(request.getAttribute("disId").toString() != null)
			{
				int disId = Integer.parseInt(request.getAttribute("disId").toString());
				System.out.println(disId);
				List<UpazillaModel> upazillaModels = ad.getUpazillaByDistrictId(disId);
				
				request.setAttribute("upazilla", upazillaModels);
			}
	...
	<div class="container-box big">
		<table align="center">
			...
			<tr>
	 		...
	 		<c:forEach items= "${upazilla}" var="area">
			<tr>
				<td>${area.upazillaId}</td>
			<td><a href="./areas?action=viewupa&upaId=${area.upazillaId}">${area.upazillaBangaName}</a></td>
				<td>
					<a class="blue-button small" href="./areas?action=update&upaId=${area.upazillaId}">Edit</a>
					<a class="red-button small" href="./areas?action=delete&upaId=${area.upazillaId}">Delete</a>
				</td>
			</tr>
			</c:forEach> 
		</table>
